...
function loadState() {
    ...
    if (savedState) {
        ...
        generateTable(); // Pokud není uložený stav, vygeneruj prázdnou tabulku
    }
}

// Event listener pro změnu velikosti mřížky
document.getElementById("gridSize").addEventListener("change", () => {
    gridSize = parseInt(document.getElementById("gridSize").value);
    generateTable();
    selectedWords = []; // Vymaže aktuální slova při změně velikosti
    bingoGrid = [];     // Vymaže Bingo kartu při změně velikosti
    localStorage.removeItem("bingoState"); // Smaže uložený stav, protože velikost mřížky se změnila
});
